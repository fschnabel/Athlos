  ...
  return (
    <View style={styles.container}>
                  ...
                  {eligibleAthletes.length === 0 ? (
                    ...
                    <View style={styles.optionList}>
                      {eligibleAthletes.map((athlete) => {
                        ...
                        return (
                          <Pressable key={key} onPress={() => toggleSelection(category.id, discipline, athlete.id)} style={[styles.optionRow, checked && styles.optionRowChecked]}>
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, spacing.sm) }]}>
        {invitation.status !== "accepted" ? <AppButton label={t("invitations.reject")} variant="danger" onPress={() => void handleReject()} loading={rejecting} /> : null}
        <AppButton label={invitation.status === "accepted" ? t("invitations.updateRegistrations") : t("invitations.accept")} onPress={() => void handleAccept()} loading={saving} />
